
LICENSE
The original template was created by Inovatik (Pavo) and is offered for free use under the open source MIT license for any type and any number of projects. This fork/customization renames the project to "Momentum" and adapts copy for an app focused on reducing procrastination.


FILE STRUCTURE
- index.html holds the entire content with Tailwind CSS utility classes
- article.html, privacy.html and terms.html are additional pages (adaptados a Momentum)
- Tailwind CSS is imported via CDN link
- css/styles.css custom css styling
- js/scripts.js custom js code
- images folder contains all the images
- The rest are files specific to different frameworks and dependencies


FRAMEWORKS & DEPENDENCIES
- Tailwind CSS https://tailwindcss.com/
- jQuery https://jquery.com/ 
- jQuery Easing https://jqueryui.com/easing/
- Magnific Popup https://dimsemenov.com/plugins/magnific-popup/
- Swiper https://swiperjs.com/
- Font Awesome for icons https://fontawesome.com/


IMAGES
All images are included in the download package and can be reused in your projects. The ones mentioned below come for outside resources. The ones not mentioned come from inside resources like created by Inovatik or purchased special license from authors. Either way you can use them for free in your project if you want.
- Mobile app UI kit used in screenshots: https://dribbble.com/shots/2398307-Phoenix-UI-Vol-1-for-iPhone-6-Free-PSD-Sketch
- Header smartphone mockup: in the Resources folder
- Header team members: https://www.pexels.com/photo/photo-of-people-standing-near-blackboard-3184393/
- Features icons: https://remixicon.com/ 
- Pricing background: https://www.pexels.com/photo/shallow-focus-photo-of-people-discussing-3182826/
- Testimonials: https://www.pexels.com/photo/photo-of-people-standing-near-blackboard-3184393/
- Conclusion smartphone mockup: in the Resources folder
- Article details image large: https://www.pexels.com/photo/photo-of-imac-near-macbook-1029757/ 
- Article details image small: https://www.pexels.com/photo/apple-office-internet-ipad-38544/



CREDITS
- Original template by Inovatik (Pavo)
- Pixel 4 mockup by Asylab: https://www.asylab.com/  
- Phoenix Mobile App UI Kit by Adrian Chiran: https://dribbble.com/adrianchiran  
- Icons by Remixicon: https://remixicon.com/ 
- Images by Pexels: https://www.pexels.com/


-----------------------------------------------------


Change Testimonials Slider Properties
- Open for editing js/scripts.js
- Find the section /* Card Slider - Swiper */
- And then fiddle with the settings using the documentation here: https://swiperjs.com/swiper-api


-----------------------------------------------------


Change Statistics Values
- Open for editing index.html
- Find the section <!-- Statistics -->
- And then change the value of the data-count tag for each number